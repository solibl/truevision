class StrainsController < ApplicationController
  ...
  def info
    ...
    @best_dryback = @data_sheets.where(current: true, marked_for_outlier: false).where.not(first_initial_dry_back: nil).order(:first_initial_dry_back).first
    @best_average_gram_difference = @data_sheets.where(current: true, marked_for_outlier: false).where.not(average_gram_difference: nil).order(:average_gram_difference).first
